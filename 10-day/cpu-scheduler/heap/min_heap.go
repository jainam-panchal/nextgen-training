...
func (h *MinHeap[T]) Verify() bool {
	for parentIndex := range h.items {
		leftIndex := getLeftChildIndex(parentIndex)
		rightIndex := getRightChildIndex(parentIndex)

		if leftIndex < len(h.items) &&
			h.less(h.items[leftIndex], h.items[parentIndex]) {
			return false
		}

		if rightIndex < len(h.items) &&
			h.less(h.items[rightIndex], h.items[parentIndex]) {
			return false
		}
	}
...
func (h *MinHeap[T]) heapifyDown(index int) {
	...
	for {
		...
		if leftIndex <= lastIndex &&
			h.less(h.items[leftIndex], h.items[smallestIndex]) {
			smallestIndex = leftIndex
		}

		if rightIndex <= lastIndex &&
			h.less(h.items[rightIndex], h.items[smallestIndex]) {
